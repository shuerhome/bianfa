...
function parseJson<T>(res: { status: number; bodyText: string }): T {
  ...
  if (res.status < 200 || res.status >= 300) throw new IpcError(`http_${res.status}`, res.bodyText.slice(0, 200));
  try {
...
export async function fetchNotesSince(workspaceId: string, sinceVersion: number, limit = 500): Promise<NotesPage> {
  const q = new URLSearchParams({ workspace_id: workspaceId, since_version: String(sinceVersion), limit: String(limit) });
...
export async function fetchOrgNotes(orgId: string): Promise<RemoteNoteSummary[]> {
  const res = await apiRequest({ method: "GET", path: `/v1/orgs/${encodeURIComponent(orgId)}/notes`, timeoutMs: 15_000 });
