...
> **📌 2026-09-06 决定（[ADR-001](ADR-001-代码签名策略.md)）**：本节「Windows 签名」「macOS 签名」两段的行动项（第 1 周提交 Azure / Apple 身份申请）**在 v1 不执行**。Windows 不签名；macOS 用自签名证书（`infra/ci/make-selfsign-cert.sh`）。分析保留，作为以后购买时的依据；`.github/workflows/desktop.yml` 有则签、无则退。
